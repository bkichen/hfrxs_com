...
function loadbooker()
{
	...
	if(books.length){
		...
		if(i<=19){
			bookhtml+='<div class="bookbox"><div class="bookimg"><a href="/'+Math.floor(books[i][0]/1000)+'_'+books[i][0]+'/"><img src="http://www.biquge.com/files/article/image/'+Math.floor(books[i][0]/1000+1)+'/'+books[i][0]+'/'+books[i][0]+'.jpg" onerror="this.src=\'http://www.biquge.com/images/nopic.gif\'"></a></div><div class="bookinfo"><h4 class="bookname"><i class="iTit"><a href="/'+Math.floor(books[i][0]/1000)+'_'+books[i][0]+'/">'+books[i][2]+'</a></i></h4><div class="cat">分类：'+books[i][5]+'</div><div class="cl0"></div><div class="author">作者：'+books[i][4]+'</div><div class="cl0"></div><div class="update"><span>已读到：</span><a href="/'+Math.floor(books[i][0]/1000)+'_'+books[i][0]+'/'+books[i][1]+'.html">'+books[i][3]+'</a></div></div><div class="delbutton"><a class="del_but" href="javascript:removebook(\''+books[i][0]+'\')">删除</a></div></div>'
			}
		}
     	}
	else{
